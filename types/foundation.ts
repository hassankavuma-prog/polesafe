...
export type VehiclePhysicalClass = 'boda' | 'standard_car' | 'large_car' | 'van' | 'minibus' | 'shuttle' | 'bus';
export type VehicleOwnershipType = 'individual' | 'driver' | 'fleet_company' | 'school' | 'leased' | 'third_party_authorized';
...
export type VehicleServiceType = 'community' | 'school' | 'shared_carpool' | 'private_fleet';
...
export type Vehicle = { vehicleId: string; ownerType: VehicleOwnershipType; ownerAccountId?: string; ownerOrganizationId?: string; physicalClass: VehiclePhysicalClass; seatCapacity?: number; reservedSeats?: number; availableSeats?: number; luggageCapacity?: number; accessibilityFeatures?: string[]; schoolTransportEligibility?: DriverEligibility; insuranceStatus?: VehicleInsuranceStatus; inspectionStatus?: VehicleInspectionStatus; activeStatus: VehicleActiveStatus; serviceTypes?: VehicleServiceType[]; fleetProviderId?: string; createdAt?: string; updatedAt?: string; };
export type DriverVehicleAssignment = { assignmentId: string; driverId: string; vehicleId: string; status: AssignmentStatus; validFrom: string; validUntil?: string; assignedBy?: string; assignedAt?: string; };
...
export type ProviderType = 'individual_driver' | 'parent_driver' | 'fleet_operator' | 'taxi_operator' | 'van_operator' | 'bus_operator';
export type Provider = { providerId: string; providerType: ProviderType; organizationId?: string; displayName?: string; activeStatus?: ApprovalLifecycleStatus; };
