...
router.use((req, res, next) => {
  if (req.path.includes('/unmask') && !hasDispatcherAccess(req.userRole || req.user?.role || req.body.userRole)) {
    return res.status(403).json({ error: 'Dispatcher permissions required' });
  }
  next();
});
