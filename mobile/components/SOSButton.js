...
export default function SOSButton({ rideId, kidId, userRole }) {
  ...
  // Fake GPS coordinates for dev (replace with expo-location in production)
  ...
  const handlePress = () => {
    if (alertActive) {
      // Already triggered — confirm resolve
      Alert.alert(
        'Resolve Alert',
    ...
    // Start 3-second countdown for accidental presses
    setCountdown(3);
  ...
  const triggerSOS = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Alert.alert(
          '🚨 SOS Sent!',
          `Emergency alert sent. ${data.contactsNotified || 0} contact(s) notified.\n\nHelp is on the way.`,
          [{ text: 'OK' }]
        );
      } else {
      ...
      Alert.alert('Error', 'Network error. Call emergency services directly.');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity
        style={[
          styles.sosButton,
          alertActive && styles.sosActive,
          sending && styles.sosSending,
        ]}
        onPress={handlePress}
        disabled={sending}
        activeOpacity={0.7}
